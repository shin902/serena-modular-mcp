...
> 補足: 多くの場合、`$schema` はエディタの補完や静的検証のための参照です。実行時にスキーマそのものをダウンロードする必要は稀で、`"$schema": "./config-schema.json"` を削除しても動作します。ただし、エディタの補完やローカルでのバリデーションは失われるため、開発時は残しておくことを推奨します。
...
**注意**: 設定ファイルのパスは相対パス限定です（例: `./serena-config.json`）。絶対パスはサポートされていません。
